    ...
    <div :id="'select-contact-card-' + _uid">
        <div class="relative" :class="[{'fs-exclude': show.contact_selected}]">
            ...
            <div :class="show.contact_selected ? 'flex' : 'hidden'" class="absolute flex-col mt-2">
                ...
                <!--   atg display shipp address start -->
                <div class="form-text-container px-2 pt-3 pb-2" >
                    <form>
                        <div class="row">
                            <div class="form-group col-md-3" >
                                <label for="atg_discount" class="text-black text-sm font-medium">Discount</label>
                                ...
                                >
                                    <option>{{ contact.atg_discount }}</option>
                                </select>
                            </div>

                            <div class="form-group col-md-9" onclick="atgStoreValue();">
                                <label><span class="text-black text-sm font-medium">Select delivery adress:</span></label>
                                <select name="state"
                                        id="SelectedShipAddr"
                                        class="w-full text-sm px-3 py-2.5 mt-1 rounded-lg border border-light-gray text-black placeholder-light-gray bg-white disabled:bg-gray-200 focus:outline-none focus:ring-transparent focus:border-purple"
                                >
                                    <option>{{ contact.address }}</option>
                                    <option>{{ contact.ship_adresse }}</option>
                                    <option>{{ contact.atg_ship_address_2 }}</option>
                                </select>
                            </div>
                        </div>
                    </form>
                ...
                contacts.forEach(function (contact, index) {
                    if (contact.enabled) {
                        this.contact_list.push({
                            ...
                            atg_name_suffix: (contact.atg_name_suffix) ? contact.atg_name_suffix : '',
                            ship_adresse: (contact.ship_adresse) ? contact.ship_adresse : '',
                            atg_ship_address_2: (contact.atg_ship_address_2) ? contact.atg_ship_address_2 : '',
                            atg_discount: (contact.atg_discount) ? contact.atg_discount : ''
                        });
                    }
                }, this);
            })
