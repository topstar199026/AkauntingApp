<template>
    <base-input :label="title" :name="name"
        ...
        <select multiple :name="name" :id="name" class="hidden">
            <option v-for="option in sorted_options2" :key="option.key" :value="option.key">{{ option.value }}</option>
        </select>
    </base-input>
</template>
...
export default {
    ...
    props: {
        ...
        multiple: {
            type: Boolean,
            default: false,
            description: "Multiple feature status"
        },

        allSelectable: {
            type: Boolean,
            default: false,
            description: "All selectable feature status"
        },
    ...
    mounted() {
        ...
        else if (typeof this.value === 'string') this.selected = JSON.parse(this.value);
        else this.selected = this.value;
        console.log('optionsoptions', this.options, this.selected)
    ...
    methods: {
        ...
        setSortedOptions() {
            ...
            if (this.group) {
                ...
                if (!Array.isArray(created_options)) {
                    ...
                    this.sorted_options2 = [...this.sorted_options];
                    this.sorted_options.unshift(this.all_choose);
                } else {
                    created_options.forEach(function (option, index) {
                        if (typeof(option) == 'string') {
                            this.sorted_options.push({
        ...
        change() {
            ...
            console.log(members.value, this.selected);
            var isAll = this.selected.length && this.selected[0] === "0" && this.sorted_options[0].key === "0";

            if (isAll) {
                this.sorted_options.shift();
                this.selected = this.sorted_options2.map((v) => v.key);
            } else {
                var containedAll = this.sorted_options[0].key === "0";
                if (!containedAll && this.sorted_options.length - this.selected.length > 2) {
                    this.sorted_options.unshift(this.all_choose);
                } else if (containedAll && this.sorted_options.length - this.selected.length <= 3) {
                    this.sorted_options.shift();
                }
            }
            ...
            members?.options.forEach((option) => {
                let _s = this.selected.some((s) => s.toString() === option.value);
                if(_s) {
                    option.setAttribute('selected', _s);
                } else {
