    ...
    <x-slot name="body">
        ...
        <div class="sm:col-span-3 grid gap-x-8 gap-y-6">
            ...
            @endif
                ...
                {{-- add customer nbr field start --}}
            @if (! $hidePhone)
                <x-form.group.text name="customer_nbr" label="{{ trans('general.at_customer_nbr') }}" not-required />
            @endif

            @if (! $hidePhone)
                <x-form.group.text name="atg_discount" label="{{ trans('general.atg_discount') }}" not-required />
            @endif
                {{-- add customer nbr field end --}}
