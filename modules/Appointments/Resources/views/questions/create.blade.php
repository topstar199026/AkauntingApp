<x-layouts.admin>
    <x-slot name="title">{{ trans('general.title.new', ['type' => trans_choice('appointments::general.questions', 1)]) }}</x-slot>

    <x-slot name="favorite"
        title="{{ trans('general.title.new', ['type' => trans_choice('appointments::general.questions', 1)]) }}"
        icon="today"
        route="appointments.questions.create"
    ></x-slot>

    <x-slot name="content">
        <x-form.container>
            <x-form id="question" route="appointments.questions.store">
                <x-form.section>
                    <x-slot name="head">
                    ...
                    <x-slot name="body">
                        ...
                        <x-form.group.select name="question_type" label="{{ trans('appointments::general.question_types') }}" :options="$question_type" change="onSelectType" form-group-class="sm:col-span-3" />
