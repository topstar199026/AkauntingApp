...
class ProjectLineChart extends Widget
{
    ...
    public $default_name = 'projects::general.widgets.cash_flow_by_project';
    ...
    public function show($project = null)
    {
        ...
        $incomes = array_values($this->calculateTotals('income', $project));
        $expenses = array_values($this->calculateTotals('expense', $project));
        $profit = array_values($this->calculateProfit($incomes, $expenses));
        ...
        $chart->setType('line')
            ...
            ->setColors($colors)
            ->setDataset(trans('general.incoming'), 'column', $incomes)
            ->setDataset(trans('general.outgoing'), 'column', $expenses)
            ->setDataset(trans_choice('general.profits', 1), 'line', $profit);

        $incoming_amount = money(array_sum($incomes), setting('default.currency'), true);
        $outgoing_amount = money(abs(array_sum($expenses)), setting('default.currency'), true);
    ...
    private function calculateTotals($type, $project): array
    {
        $totals = [];
        ...
        if ($project && in_array($type, ['income', 'expense'])) {
            $model = $model->whereIn('id', $this->getTransactionIds($type, $project));
        }

        $model = $model->whereBetween('paid_at', [$this->start_date, $this->end_date])
            ->isNotTransfer();

        $items = $this->applyFilters($model)->get();
    ...
    private function getTransactionIds(string $type, $project)
    {
        $ids = $project->financials()->type(Transaction::class)->pluck('financialable_id');

        return Transaction::whereIn('id', $ids)->type($type)->pluck('id');
    }
