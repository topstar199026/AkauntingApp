<x-layouts.admin>
    <x-slot name="title">{{ trans('general.title.new', ['type' => trans_choice('inventory::general.transfer_orders', 1)]) }}</x-slot>

    <x-slot name="favorite"
        title="{{ trans('general.title.new', ['type' => trans_choice('inventory::general.transfer_orders', 1)]) }}"
        icon="local_shipping"
        route="inventory.transfer-orders.create"
    ></x-slot>

    <x-slot name="content">
        <x-form.container>
            <x-form id="transfer-order" route="inventory.transfer-orders.store">
                <x-form.section>
                    <x-slot name="head">
                    ...
                    <x-slot name="body">
                        ...
                        <x-form.group.text name="transfer_order_number" label="{{ trans_choice('inventory::transferorders.transfer_order_number', 1) }}" :value="old('transfer_order_number', $number)"/>
                                    ...
                                    <x-table.tbody>
                                        <x-table.tr class="relative flex items-center px-1 group/actions border-b" v-for="(row, index) in form.items" ::index="index">                          
                                            ...
                                            <x-table.td class="w-3/12">
                                                <x-form.group.text name="items[][item_quantity]" disabled data-item="item_quantity" v-model="row.item_quantity" />
                                            </x-table.td>
