    ...
    public function handle(): ItemGroup
    {
        \DB::transaction(function () {
            ...
            if ($this->request->variants) {
                ItemGroupVariant::where('item_group_id', $this->model->id)->delete();
                ...
                foreach ($this->request->variants as $variant) {
                    ...
                    $this->dispatch(new CreateItemGroupVariant($variant_data));
                }
            }
            ...
            if ($this->request->has('items')) {
                ...
                foreach ($this->request->items as $item_data) {
                    ...
                    $item_data['items'][0] = [
                        ...
                        'default_warehouse' => 1,
                        'sku' => $item_data['sku'],
                        'barcode' => $item_data['barcode'] ?? null,
                    ];
                    ...
                    if (!isset($item_data['variant_value_id'])) {
                        $item_data['variant_value_id'] = $variant_data['variant_values'];
                    }
                    ...
                    $this->dispatch(new UpdateItemGroupItem($main_request, $group_item));
                }
            }
        });
