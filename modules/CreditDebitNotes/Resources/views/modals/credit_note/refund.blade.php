...
>
    ...
    <div class="grid sm:grid-cols-6 gap-x-8 gap-y-6 my-3.5">
        ...
        <x-form.group.money name="amount" label="{{ trans('general.amount') }}" value="{{ $credit_note->amount }}" autofocus="autofocus" :currency="$currency" dynamicCurrency="currency" />

        <x-form.group.account change="onChangePaymentAccount" />

        <x-form.group.text name="currency" label="{{ trans_choice('general.currencies', 1) }}" value="{{ $credit_note->currency->name }}" not-required disabled />

        <x-form.group.textarea name="description" label="{{ trans('general.description') }}" rows="3" not-required />

        <x-form.group.payment-method />

        <x-form.group.category type="expense" />

        <x-form.input.hidden name="document_id" :value="$credit_note->id" />
        <x-form.input.hidden name="amount" :value="$credit_note->amount" />
        <x-form.input.hidden name="currency_code" :value="$credit_note->currency_code" />
        <x-form.input.hidden name="currency_rate" :value="$credit_note->currency_rate" />
...
{{--        <x-form.input.hidden name="type" value="credit_note_refund" />--}}
    </div>
</x-form>
