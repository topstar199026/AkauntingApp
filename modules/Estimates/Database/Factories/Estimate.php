...
class Estimate extends DocumentFactory
{
    ...
    public function estimate(): Factory
    {
        ...
        $issued_at = $this->getRawAttribute('issued_at');

        $expire_at = Date::parse($issued_at)
                         ->addDays($this->faker->randomNumber(3) + 1)
                         ->format('Y-m-d');
        ...
        return $this->afterCreating(function (Document $document) {
            ...
            switch ($init_status) {
                case 'received':
                    ...
                    break;
                case 'sent':
                    event(new DocumentMarkedSent($updated_document));
