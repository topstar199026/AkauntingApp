...
class Estimates extends Controller
{
    ...
    public function preview(Document $estimate)
    {
        if (empty($estimate)) {
            return redirect()->route('login');
        }

        return view('estimates::portal.estimates.preview', compact('estimate'));
    }
    ...
    public function signed(Document $estimate)
    {
        ...
        if (empty(user()) || user()->id == $estimate->contact->user_id) {
            event(new DocumentViewed($estimate));
        }
