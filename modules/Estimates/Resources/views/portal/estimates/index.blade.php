<x-layouts.portal>
    ...
    <x-slot name="content">
        @if ($estimates->count() || request()->get('search', false))
            <x-index.container>
                <x-index.search search-string="portal-estimates" />

                <x-table>
                    <x-table.thead>
                        <x-table.tr class="flex items-center px-1">
                            <x-table.th kind="action"></x-table.th>
                            ...
                            <x-table.th class="w-4/12 hidden sm:table-cell">
                                ...
                                <x-slot name="first">
                                    <x-sortablelink column="expire_at" title="{{ trans('estimates::general.expiry_date') }}" />
                                </x-slot>
                            ...
                            <x-table.tr href="{{ route('portal.invoices.show', $item->id) }}">
                                ...
                                <x-table.td class="w-3/12  sm:table-cell">
                                    ...
                                    <x-slot name="first" class="relative w-20 font-normal group" data-tooltip-target="tooltip-information-{{ $item->id }}" data-tooltip-placement="left" override="class,data-tooltip-target,data-tooltip-placement">
                                        ...
                                        <x-documents.index.information :document="$item" show-route="portal.estimates.estimates.show"/>
                                    </x-slot>
