        ...
        <x-slot name="buttons">
            @if($estimate->status !== 'expired')
                @if($estimate->status !== 'approved')
                    <x-link override="class" href="{{ $approveAction }}" class="bg-green text-white px-3 py-1.5 mb-3 sm:mb-0 rounded-lg text-sm font-medium leading-6 hover:bg-green-700">
                        {{ trans('estimates::general.approve') }}
                    </x-link>
                @endif
                @if($estimate->status !== 'refused')
                    <x-link override="class" href="{{ $refuseAction }}" class="bg-red text-white px-3 py-1.5 mb-3 sm:mb-0 rounded-lg text-sm font-medium leading-6 hover:bg-red-700">
                        {{ trans('estimates::general.refuse') }}
                    </x-link>
                @endif
            @endif
            @stack('button_pdf_start')
            <x-link href="{{ $pdfAction }}" class="px-3 py-1.5 mb-3 sm:mb-0 bg-gray-100 hover:bg-gray-200 rounded-lg text-sm font-medium leading-6">
                {{ trans('general.download') }}
            </x-link>
            @stack('button_pdf_end')

            @stack('button_print_start')
            <x-link href="{{ $printAction }}" target="_blank" class="px-3 py-1.5 mb-3 sm:mb-0 bg-gray-100 hover:bg-gray-200 rounded-lg text-sm font-medium leading-6">
                {{ trans('general.print') }}
            </x-link>
