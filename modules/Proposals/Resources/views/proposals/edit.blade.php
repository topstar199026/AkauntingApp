<x-layouts.admin>
    <x-slot name="title">
        {{ trans('general.title.edit', ['type' => trans_choice('proposals::general.proposals', 1)]) }}
    </x-slot>
    ...
    <x-slot name="content">
        ...
        <x-form.container class="w-full" override="class">
            <x-form id="proposal" method="PATCH" :route="['proposals.proposals.update', $proposal->id]" :model="$proposal">
                <x-form.section>
                    <x-slot name="body">
                        <x-form.group.text name="description" label="{{ trans('general.description') }}" />

                        <x-form.group.select name="template_id" label="{{ trans_choice('general.templates', 1) }}" :options="$templates" :selected="$proposal->template_id" change="onChangeTemplate" not-required />

                        @if(module('estimates'))
                            <x-form.group.select name="estimates_id" label="{{ trans_choice('estimates::general.estimates', 1) }}" :options="$estimates" :selected="$proposal->estimates_id" not-required />
                        @endif

                        <x-form.input.hidden name="content_html" value="{{ $proposal->content_html }}" v-model="form.content_html" />
                        <x-form.input.hidden name="content_css" value="{{ $proposal->content_css }}" v-model="form.content_css" />
                        <x-form.input.hidden name="content_components" value="{{ $proposal->content_components }}" v-model="form.content_components" />
                        <x-form.input.hidden name="content_style" value="{{ $proposal->content_style }}" v-model="form.content_style" />
                    </x-slot>
                </x-form.section>
    ...
    @push('scripts_start')
        ...
        <script type="text/javascript">
            ...
            var editor = grapesjs.init({
                ...
                components: {!! $proposal->content_components !!},
                style: {!! $proposal->content_style !!},
