<x-layouts.admin>
    <x-slot name="title">
        {{ trans('general.title.edit', ['type' => trans_choice('general.templates', 1)]) }}
    </x-slot>
    ...
    <x-slot name="content">
        ...
        <x-form.container class="w-full" override="class">
            <x-form id="template" method="PATCH" :route="['proposals.templates.update', $template->id]" :model="$template">
                <x-form.section>
                    <x-slot name="body">
                        ...
                        <x-form.input.hidden name="content_html" value="{{ $template->content_html }}" v-model="form.content_html" />
                        <x-form.input.hidden name="content_css" value="{{ $template->content_css }}" v-model="form.content_css" />
                        <x-form.input.hidden name="content_components" value="{{ $template->content_components }}" v-model="form.content_components" />
                        <x-form.input.hidden name="content_style" value="{{ $template->content_style }}" v-model="form.content_style" />
    ...
    @push('scripts_start')
        ...
        <script type="text/javascript">
            ...
            var editor = grapesjs.init({
                ...
                components: {!! $template->content_components !!},
